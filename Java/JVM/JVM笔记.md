...
- 方法区
  - JVM只有一个方法区，所有线程共享；
  - 方法区实际也是堆（元空间里），只是用于存储类、常量相关的信息；
  - 方法区用来存放程序中永远不变或唯一的内容（类信息、静态变量、字符串常量）
  - 1.8之后方法区放在元空间里，所以运行时常量池在方法区-元空间里；但是类常量池和字符串常量池在堆里
...
- 类文件中常量池（The Constant Pool）
  ...
  - 诞生时间：编译时
  - 内容概要：
    - 每一个Java类被编译后，就会形成一份class文件；class文件中除了包含**类的版本、字段、方法、接口**等描述信息外，还有一项信息就是**常量池**(constant pool table)，用于存放编译器生成的各种 字面量 (Literal)和 符号引用 (Symbolic References)，每个class文件都有一个class常量池。
    - 类常量池中包含字面量和符号引用，存的不是对象实例，而是对象的符号引用值
...
- 运行时常量池（The Run-Time Constant Pool）
  ...
  - 诞生时间：JVM运行加载类文件后
  - 内容概要：
    - 字面量可以动态的添加(String类的intern()),符号引用可以被解析为直接引用
...
public class Test {
    public static void main(String[] args) {
        String s1 = new String("abc"); //记录堆内存对象的地址
...
`equals()`来比较的是两个对象是否相等，由于所有的类都是继承自`java.lang.Object`类的，在Object中的基类中定义了一个equals的方法，这个方法的**初始行为是比较对象的内存地址**，但**String类中重写了equals方法**， 比较的是字符串的内容 ，而不再是比较类在堆内存中的存放地址了。
